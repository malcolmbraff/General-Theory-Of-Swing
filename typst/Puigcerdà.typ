...
The paper proceeds in six sections following this introduction. @mirror reframes the _mirror effect_ as a substitution rule, diagnoses its failure under iteration, and extracts the consistency condition that will structure the analysis throughout. @phi corrects this failure with a _swinging substitution_ whose consistency condition selects the golden ratio $phi.alt$ — the canonical Fibonacci morphism of the combinatorial literature on Sturmian words. @rho returns to $sqrt(2)$ via a second binary rule, then extends the framework to a ternary alphabet with a rule whose consistency condition selects the plastic ratio $rho$ and whose iterated output matches a pattern found in traditional Brazilian rhythms. @perception examines the perceptual limits that bound how deeply fractal swing can be realised in performance, and shows that these limits act as a structural selector among the substitution rules admissible in principle — yielding the triad ${rho, sqrt(2), phi.alt}$ as the structurally and perceptually admissible outcome of the framework. @future defines fractal swing as a structural category, outlines directions for future research under the heading of _tempered fractal swings_, and introduces the concert _Morph Congas_ as a practical realisation of the framework. @conclusion concludes and transitions to the concert that follows.
